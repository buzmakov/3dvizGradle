# Разворот сайта
* Использовал как базу [Google cloud](https://cloud.google.com/). Там при регистрации дают стартовые 300$
...
* для сборки и тестирования есть набор шелл скриптов в корне проекта. Это является отправной точкой для работы с проектом:
    ...
    * Далее идут 2 скрипта которые имеют одинаковую логику. делают сборку, собирают докер контейнер, публикуют в [Dockerhub](https://hub.docker.com/r/opaapao/3dvize/tags/). Но в разных форматах:
...
* Для сборки джава артефактов используется инструмент [Gradle](https://gradle.org/). Инструкцию по основным методом можно прочитать [здесь](https://guides.gradle.org/building-java-applications/) Для него ключевые файлы, из которых он будет делать сборку:
...
*  после публикации на докерхабе, можно создать kubernates проект в google cloud. Наиболее близкую инструкцию можно найти [здесь](https://cloud.google.com/kubernetes-engine/docs/tutorials/hello-app)
Если использовать вариант, когда данные снаружи и аттачатся через вольюм, то [вот инструкция](https://cloud.google.com/kubernetes-engine/docs/tutorials/persistent-disk), как это можно сделать
* Пример развернутого приложения [35.239.213.226](http://35.239.213.226/)
...
* Для всех запросов был добавлен хедер ("Access-Control-Allow-Origin": "*") чтобы можно было тестировать локалоно фронтенд отдельно от бекенда.
Не был вынесен отдельно в пост шаг, а проставлен в каждом ресту, тк проставлялся не у всех запросов
...
* Реализована работа только для массивов float, тк он использовался в работе. Если нужны какие-то другие типы, нужно наследоваться от класса H5Object.
Сделать как-то универсально и красиво через дженерики не получилось из-за особенности библиотки jhdf5, где для каждого типа свои классы
...
* Так же как и нет каких-то базовых вещей, типа нахождения минимума/максимума.
Это реализовано в классе  com.rbtm.reconstruction.Utils:CustomArrayUtils
...
Для визуализации сладов реализована возможность накладывания произвольного набора фильтров.
Все минимальные-максимальные значения привязаны к размеру изображения и присваиваются в файле filterGenerator.
Порядок фильров имеет значение, могут повторяться. Посылается на сервер в json формате, при каждом изменении слоя иди фильтра
Не было реализовано блокирования опций, если не выбран объект. Поэтому при изменении параметров автоматически выбирается первый в списке
...
* **scripts/filterGenerator.js** - скрипт который генерирует новый фильтр.
    Создает новый елемент списка со следующими полями:
    * Имя
    * Мин. значение
    * Слайдер
