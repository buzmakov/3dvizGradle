...
* для сборки и тестирования есть набор шелл скриптов в корне проекта. Это является отправной точкой для работы с проектом:
    ...
    * Далее идут 2 скрипта которые имеют одинаковую логику. делают сборку, собирают докер контейнер, публикуют в [Dockerhub](https://hub.docker.com/r/opaapao/3dvize/tags/). Но в разных форматах:
        * **withDataBuild.sh** - публекует вместе с данными
        * **withoutDataBuild** - публикует без данных. Добавляет точку монтирования вольюма, куда можно приатачить внешний вольюм с данными через volume bind

* публикация есть в 2-х форматах:
    * со всеми данными внутри
    * без данных, данные аттачатся к докеру через volume bind
*  после публикации на докерхабе, создается kubernates проект в google cloud. Наиболее близкую инструкцию можно найти [здесь](https://cloud.google.com/kubernetes-engine/docs/tutorials/hello-app)
